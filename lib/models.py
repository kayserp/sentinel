...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, iqcashd):
        golist = iqcashd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_iqcashd(iqcashd, item)
    ...
    @classmethod
    def import_gobject_from_iqcashd(self, iqcashd, rec):
        import decimal
        import iqcashlib
        import inflection
        ...
        # shim/iqcashd conversion
        object_hex = iqcashlib.SHIM_deserialise_from_iqcashd(object_hex)
        objects = iqcashlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from iqcashd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from iqcashd, with every run
        ...
        # get/create, then sync payment amounts, etc. from iqcashd - IQcashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(iqcashd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from iqcashd! %s" % e)
            govobj.vote_delete(iqcashd)
            return (govobj, None)
    ...
    def vote_delete(self, iqcashd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(iqcashd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, iqcashd, signal, outcome):
        import iqcashlib
        ...
        output = iqcashd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = iqcashlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(iqcashd, signal)

    def sync_network_vote(self, iqcashd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = iqcashd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 iqcash addr, non-multisig
            if not iqcashlib.is_valid_iqcash_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid IQcash address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (iqcashlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/IQcashDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import iqcashlib
        obj_data = iqcashlib.SHIM_serialise_for_iqcashd(self.serialise())
    ...
    def prepare(self, iqcashd):
        try:
            object_hash = iqcashd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not iqcashlib.is_valid_iqcash_address(addr, config.network):
    ...
    def hash(self):
        import iqcashlib
        return iqcashlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, iqcashd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - iqcashd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, iqcashd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - iqcashd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, iqcashd):
        now = int(time.time())
        return (self.created_at < (now - iqcashd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, iqcashd):
        if self.is_expired(iqcashd):
            return False

        return True

    def is_deletable(self, iqcashd):
        if self.is_expired(iqcashd):
            return True
