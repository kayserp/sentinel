...
sentinel_cfg = IQcashConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_iqcashd_proto_version_with_sentinel_ping = 70207


def get_iqcash_conf():
    home = os.environ.get('HOME')

    iqcash_conf = os.path.join(home, ".iqcashcore/iqcash.conf")
    if sys.platform == 'darwin':
        iqcash_conf = os.path.join(home, "Library/Application Support/IQcashCore/iqcash.conf")

    iqcash_conf = sentinel_cfg.get('iqcash_conf', iqcash_conf)
