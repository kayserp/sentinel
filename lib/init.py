...
def has_iqcash_conf():
    ...
    valid_iqcash_conf = False

    # ensure iqcash_conf exists & readable
    #
    # if not, print a message stating that IQcash Core must be installed and
    # configured, including JSONRPC access in iqcash.conf
    try:
        f = io.open(config.iqcash_conf)
        valid_iqcash_conf = True
    except IOError as e:
        print(e)

    return valid_iqcash_conf
...
def main():
    ...
    if not has_iqcash_conf():
        print("IQcashCore must be installed and configured, including JSONRPC access in iqcash.conf")
        sys.exit(1)
