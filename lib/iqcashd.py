...
class IQcashDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some iqcashd methods
    ...
    @classmethod
    def from_iqcash_conf(self, iqcash_dot_conf):
        from iqcash_config import IQcashConfig
        config_text = IQcashConfig.slurp_config_file(iqcash_dot_conf)
        creds = IQcashConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = iqcashlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = iqcashlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = iqcashlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_iqcashd_proto_version_with_sentinel_ping)
