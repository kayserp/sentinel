...
def is_valid_iqcash_address(address, network='mainnet'):
    ...
    iqcash_version = 140 if network == 'testnet' else 58
    ...
    try:
        ...
        # rescue from exception, not a valid IQcash address
        return False

    if (address_version != iqcash_version):
        return False
...
# shims 'til we can fix the iqcashd side
def SHIM_serialise_for_iqcashd(sentinel_hex):
    ...
    # superblock => "trigger" in iqcashd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # iqcashd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    iqcashd_hex = serialise(obj)
    return iqcashd_hex


# shims 'til we can fix the iqcashd side
def SHIM_deserialise_from_iqcashd(iqcashd_hex):
    from models import TALCASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(iqcashd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('iqcash.conf')
