...
class IQcashConfig():
    ...
    def slurp_config_file(self, filename):
        # read iqcash.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is iqcash.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from iqcash.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard IQcash defaults...
        default_port = 13013 if (network == 'mainnet') else 18013

        # use default port for network if not specified in iqcash.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from iqcash.conf
