...
def test_valid_iqcash_address():
    from iqcashlib import is_valid_iqcash_address

    main = valid_iqcash_address()
    test = valid_iqcash_address('testnet')

    assert is_valid_iqcash_address(main) is True
    assert is_valid_iqcash_address(main, 'mainnet') is True
    assert is_valid_iqcash_address(main, 'testnet') is False

    assert is_valid_iqcash_address(test) is False
    assert is_valid_iqcash_address(test, 'mainnet') is False
    assert is_valid_iqcash_address(test, 'testnet') is True


def test_invalid_iqcash_address():
    from iqcashlib import is_valid_iqcash_address

    main = invalid_iqcash_address()
    test = invalid_iqcash_address('testnet')

    assert is_valid_iqcash_address(main) is False
    assert is_valid_iqcash_address(main, 'mainnet') is False
    assert is_valid_iqcash_address(main, 'testnet') is False

    assert is_valid_iqcash_address(test) is False
    assert is_valid_iqcash_address(test, 'mainnet') is False
    assert is_valid_iqcash_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from iqcashlib import elect_mn
...
def test_parse_masternode_status_vin():
    from iqcashlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % iqcashlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(iqcashlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(iqcashlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(iqcashlib.blocks_to_seconds(16616)) == 2612035
